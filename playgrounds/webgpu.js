...
function assertMat(a, b, label = "") {
  ...
  for (const i in a) {
    if (a[i] !== b[i]) {
      throw new Error(
        `[${label}] Assertion failed. Arrays not equal. First divergence at index ${i}, expected: ${
          b[i]
        } , got: ${a[i]}`,
      );
    }
...
{
  const result = await addMatrixWebGpu({ shape: [1, 1], data: mat1AF32 }, {
    shape: [1, 1],
    data: mat1BF32,
  });
...
{
  const result = await addMatrixWebGpu({ shape: [2, 2], data: mat2AF32 }, {
    shape: [2, 2],
    data: mat2BF32,
  });
...
{
  const result = await addMatrixWebGpu({ shape: [4, 4], data: mat4AF32 }, {
    shape: [4, 4],
    data: mat4BF32,
  });
...
{
  const result = await addMatrixWebGpu({ shape: [8, 8], data: mat8AF32 }, {
    shape: [8, 8],
    data: mat8BF32,
  });
...
{
  const result = await addMatrixWebGpu({ shape: [16, 16], data: mat16AF32 }, {
    shape: [16, 16],
    data: mat16BF32,
  });
...
{
  const result = await addMatrixWebGpu(
    { shape: [256, 256], data: mat256AF32 },
    { shape: [128, 128], data: mat256BF32 },
  );
...
}
{
  const result = await addMatrixWebGpu(
    {
      shape: [1024, 1024],
      data: new Float32Array(getMatFlat(1024, 1024, normalSampler(1e9))),
    },
    {
      shape: [1024, 1024],
      data: new Float32Array(getMatFlat(1024, 1024, normalSampler(1e9))),
