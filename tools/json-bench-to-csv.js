...
for (const bench of json.benches) {
  stratOut += `${bench.name},${bench.results[0].ok.min?.toFixed(0)},${
    bench.results[0].ok.max?.toFixed(0)
  },${bench.results[0].ok.avg.toFixed(0)},${bench.results[0].ok.p75?.toFixed(0)},${
    bench.results[0].ok.p99?.toFixed(0)
  },${bench.results[0].ok.p995?.toFixed(0)}\n`;
}

Deno.writeTextFileSync(
  `./temp/bench-by-strat.${web ? "web" : "deno"}.csv`,
  stratOut,
);
...
for (const group of groups) {
  ...
  sizeOut += group + "," + strats.map((strat) =>
    stratsInGroup.find((s) =>
      s.strategy === strat
    ).results[0].ok.avg
  ).join(",") + "\n";
}
...
Deno.writeTextFileSync(
  `./temp/bench-by-size.${web ? "web" : "deno"}.csv`,
  sizeOut,
);
